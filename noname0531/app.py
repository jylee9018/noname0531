...
def train_model(data):
    ...
    model = xgb.XGBRegressor(
        objective="reg:squarederror", n_estimators=1000, learning_rate=0.01, max_depth=5
    )
...
def predict_stock_prices(model, ticker):
    future_dates = pd.date_range(
        start="2024-06-01", end="2024-12-30", freq="B"
    )  # Business days
    ...
    predictions = model.predict(
        future_data[["Year", "Month", "Day", "Open", "High", "Low", "Volume"]]
    )
